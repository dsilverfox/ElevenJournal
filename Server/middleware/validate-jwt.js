...
const validateJWT = async (req, res, next) => {
    if(req.method == "OPTIONS") { //this is a conditional statement checking the method of the request (Post, Get, Put, Delete.)
        next(); //next is a nested middleware function to pass control to the next middleware function. (Preflight?)
    } else if ( //if we are dealing with POST, GET, PUT, or DELETE his checks for authorization AND to determine if it incldes "BEARER"
        ...
        const payload = authorization //pulls the value of authorization and stores it in payload
            ...
            : authorization, process.env.JWT_SECRET //allows payload to equal a truthy value.
            )
            : undefined; //This gets stored in payload if the jwt.verify function returns a falsy value.
            console.log("payload -->", payload);

            if(payload) { //this function triggers if payload is truthy (something other than null, 0, false, or undefined)
